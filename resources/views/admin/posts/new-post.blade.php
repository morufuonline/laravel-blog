<x-admin-app-layout>
    ...
    <div class="py-4 min-h-500">
        ...
        <form action="{{ url('admin/posts/store') }}" method="post" runat="server" autocomplete="off">  
        ...
        <div class="py-2">
        <input type="text" name="title" id="title" class="form-control sm:rounded-lg" placeholder="Enter Title" required value="{{ old('title') }}">
        </div>
        @error('title')
        ...
        <div class="py-2">
        <textarea type="text" rows="7" name="body" id="body" class="form-control sm:rounded-lg" placeholder="Enter details" required>{{ old('body') }}</textarea>
        </div>
        @error('body')
        <p class="required">{{$message}}</p>
        @enderror
